...
def main():
    ...
    if res: 
        ...
        #################################
        optifyl = r"C:\binSys\Optimisation\ForNAft\conf_opts_RUN_DIR\conf_opts_OPTRES"
        ...
        _DEST = r"C:\binSys\Optimisation\ForNAft\RunFold"
        cntfyl = r"C:\binSys\Optimisation\ForNAft\RunFold\count.txt"
        _countFile = open(cntfyl, 'r')
        
        _Count = int(_countFile.read())
        _countFile.close()
        _DESTFOLD = os.path.join(_DEST,str(_Count))
        # copying outputs to runfold
        copyDir(r"C:\binSys\Optimisation\ForNAft\conf_opts_RUN_DIR",_DESTFOLD)
        _countFile = open(cntfyl, 'w')
        _countFile.write(str(_Count+1))
        _countFile.close()
        # To avoid shipflow file read error, each output is deleted
        try:
            shutil.rmtree(r"C:\binSys\Optimisation\ForNAft\conf_opts_RUN_DIR")
        ...
        if sfres:
            ...
            print "Error in writing out put"
            outf.write( "FAIL")
